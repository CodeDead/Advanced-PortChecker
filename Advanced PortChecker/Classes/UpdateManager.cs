    ...
    internal class UpdateManager
    {
        ...
        internal async void CheckForUpdate(bool showErrors, bool showNoUpdates)
        {
            try
            {
                ...
                {
                    if (MessageBox.Show("Version " + _update.GetUpdateVersion() + " is now available!" + Environment.NewLine + "Would you like to download this version?", "Advanced PortChecker", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                ...
                else
                {
                    if (showNoUpdates)
                    {
                        MessageBox.Show("No updates are currently available", "Advanced PortChecker", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
            }
            catch (Exception ex)
